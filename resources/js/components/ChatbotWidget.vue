<template>
  <div class="fixed bottom-6 right-6 z-50 flex flex-col items-end font-sans">
    ...
    <!-- Chat Window -->
    <div v-if="isOpen" class="bg-surface-container-lowest w-[300px] sm:w-[340px] h-[480px] max-h-[80vh] rounded-2xl shadow-[0_10px_40px_rgba(0,0,0,0.2)] flex flex-col overflow-hidden border border-outline-variant transition-all duration-300 transform origin-bottom-right">
      ...
      <div class="bg-primary text-on-primary p-3 flex justify-between items-center shadow-md z-10">
        ...
        <button @click="isOpen = false" class="w-7 h-7 flex items-center justify-center rounded-full hover:bg-white/20 transition-colors cursor-pointer">
      ...
      <div ref="messagesContainer" class="flex-1 p-3 overflow-y-auto flex flex-col bg-white scroll-smooth">
        ...
        <div v-for="(msg, index) in messages" :key="index" :class="['flex w-full gap-2 mb-3', msg.role === 'user' ? 'justify-end' : 'justify-start']">
          ...
          <!-- Bong bóng tin nhắn -->
          <div :class="['max-w-[75%] rounded-2xl p-2.5 shadow-sm', 
            ...
            <div v-if="msg.data && msg.data.length > 0" class="mt-3 flex overflow-x-auto gap-2 pb-2 -mx-1 px-1 snap-x product-scrollbar">
              <router-link v-for="product in msg.data" :key="product.id" :to="`/post/${product.slug}`"
                ...
                <div class="w-full h-20 bg-gray-100 flex items-center justify-center overflow-hidden">
                   <img :src="getPrimaryImage(product)" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" alt="Product Image" />
                </div>
                
                <div class="p-2 flex flex-col gap-1">
                  <span class="font-bold text-on-surface text-[11px] line-clamp-2 leading-snug h-7 group-hover:text-primary transition-colors">{{ product.title }}</span>
    ...
    <div v-if="!isOpen" class="relative flex flex-col items-end">
      ...
      <transition name="fade">
        <div v-if="showSpeechBubble" 
          ...
          <button @click.stop="showSpeechBubble = false" class="absolute top-1.5 right-1.5 w-5 h-5 rounded-full flex items-center justify-center text-gray-400 hover:text-gray-700 hover:bg-gray-100 opacity-0 group-hover:opacity-100 transition-all z-10 cursor-pointer">
          ...
          <div class="flex-1 mt-0.5 overflow-hidden pr-3">
            ...
            <transition name="text-fade" mode="out-in">
              <p :key="currentBubbleIndex" class="text-[13px] text-gray-600 leading-snug">{{ bubbleTexts[currentBubbleIndex] }}</p>
            </transition>
          </div>
      ...
      <button @click="isOpen = true; showSpeechBubble = false" class="w-14 h-14 bg-primary text-on-primary rounded-full flex items-center justify-center shadow-lg hover:shadow-xl hover:scale-105 transition-all z-10 overflow-hidden relative group cursor-pointer mt-4">
...
const getPrimaryImage = (product) => {
    return 'https://via.placeholder.com/150x100?text=S%E1%BA%A3n+Ph%E1%BA%A9m';
};
