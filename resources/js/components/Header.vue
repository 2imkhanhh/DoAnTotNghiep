    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center h-16">
            ...
            <div class="flex-shrink-0 flex items-center">
                <router-link to="/" class="flex items-center gap-2 text-primary hover:text-primary-container transition-colors">
            ...
            <div class="flex items-center gap-2 sm:gap-4 ml-auto">
                <router-link to="/chat" class="p-2 text-on-surface hover:text-primary hover:bg-surface-container rounded-full transition-colors hidden sm:block" title="Tin nhắn">
                    <div class="relative">
                ...
                <router-link to="/notifications" class="p-2 text-on-surface hover:text-primary hover:bg-surface-container rounded-full transition-colors hidden sm:block" title="Thông báo">
                    <span class="material-symbols-outlined">notifications</span>
                </router-link>

                <router-link to="/post-ad" class="hidden sm:flex items-center gap-1 font-bold text-primary hover:text-primary-container px-3 py-2 rounded-lg hover:bg-surface-container-low transition-colors">
                ...
                <div v-if="!authStore.isLoggedIn" class="flex items-center gap-2">
                    ...
                    <router-link v-if="$route.path !== '/register'" to="/register" class="px-5 py-2 bg-primary text-on-primary font-bold rounded-full shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all">
                        Đăng ký
                    </router-link>
                </div>
                ...
                <div v-if="authStore.isLoggedIn" class="flex items-center gap-2">
                    <div class="relative group cursor-pointer">
                        ...
                        <div class="absolute right-0 mt-2 w-48 bg-surface-container-lowest rounded-xl shadow-lg border border-outline-variant opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50">
                            <div class="py-2">
                                <router-link to="/profile" class="block px-4 py-2 text-sm text-on-surface hover:bg-surface-container-low hover:text-primary">Trang cá nhân</router-link>
