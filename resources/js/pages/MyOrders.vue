<template>
  <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    <div class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
      <div
        class="p-6 sm:p-8 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-slate-50/50">
        <div>
        ...
        <div class="relative min-w-[170px] custom-dropdown">
          <div @click="isDropdownOpen = !isDropdownOpen"
            class="bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-bold cursor-pointer flex items-center justify-between hover:border-primary transition-colors shadow-sm select-none">
          ...
          <div v-if="isDropdownOpen"
            class="absolute z-20 w-full mt-2 bg-white border border-slate-100 rounded-xl shadow-[0_10px_25px_-5px_rgba(0,0,0,0.1),0_8px_10px_-6px_rgba(0,0,0,0.1)] overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
            <div v-for="option in filterOptions" :key="option.value" @click="selectFilter(option.value)"
      ...
      <div class="p-6 sm:p-8 space-y-6">
        ...
        <div v-else class="space-y-4">
          <div v-for="order in buyerOrders" :key="order.id" @click="viewOrderDetails(order)"
            class="border border-slate-200 rounded-2xl p-5 sm:p-7 bg-white shadow-sm hover:shadow-md transition-shadow cursor-pointer">
            <div class="flex justify-between items-center mb-5 pb-4 border-b border-slate-100/80">
              <div class="flex items-center gap-2">
                <img
                  :src="order.seller?.avatar || 'https://ui-avatars.com/api/?name=' + encodeURIComponent(order.seller?.name || 'Shop') + '&background=random'"
            ...
            <div class="flex gap-4 mb-4">
              <img :src="order.post?.images?.[0]?.image_path || 'https://via.placeholder.com/100'"
              ...
              <div class="flex-1 min-w-0">
                ...
                <p class="text-xs text-on-surface-variant mb-2">Ngày đặt: {{ new
                  Date(order.created_at).toLocaleDateString('vi-VN') }}</p>
                <div class="text-error font-extrabold">{{ new Intl.NumberFormat('vi-VN').format(order.total_price ||
                  order.post?.price) }}đ</div>
              </div>
            </div>

            <div v-if="order.status === 'pending' || order.status === 'confirmed' || order.status === 'delivered'"
              class="flex justify-end gap-3 pt-5 border-t border-slate-100/80 mt-2">
              <button v-if="order.status === 'pending' || order.status === 'confirmed'"
                @click.stop="cancelBuyerOrder(order.id)"
                class="px-4 py-2 border border-error text-error font-bold rounded-lg hover:bg-error-container transition-colors text-sm cursor-pointer">
                Hủy đơn hàng
              </button>
              <button v-if="order.status === 'delivered'" @click.stop="openReviewModal(order)"
                class="px-4 py-2 bg-primary text-white font-bold rounded-lg hover:bg-primary/90 transition-colors text-sm cursor-pointer shadow-sm">
                {{ order.review ? 'Xem / Sửa đánh giá' : 'Đánh giá' }}
              </button>
            </div>
          </div>
        </div>
        ...
        <div v-if="buyerOrdersPagination?.last_page > 1" class="flex justify-center my-6 gap-2">
          <button :disabled="buyerOrdersPagination.current_page === 1"
            @click="fetchBuyerOrders(buyerOrdersPagination.current_page - 1)"
          ...
          <button :disabled="buyerOrdersPagination.current_page === buyerOrdersPagination.last_page"
            @click="fetchBuyerOrders(buyerOrdersPagination.current_page + 1)"
            class="w-10 h-10 rounded-lg font-bold transition-all bg-white text-slate-600 border border-slate-200 hover:bg-slate-50 disabled:opacity-50 flex items-center justify-center cursor-pointer disabled:cursor-not-allowed">
  ...
  <div v-if="selectedOrder" class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div
      ...
      <div class="p-6 overflow-y-auto grow custom-scrollbar space-y-6">
        ...
        <div>
          ...
          <div class="flex gap-4 p-4 border border-slate-100 rounded-xl bg-slate-50/30">
            <img :src="selectedOrder.post?.images?.[0]?.image_path || 'https://via.placeholder.com/100'"
              class="w-24 h-24 rounded-lg object-cover border border-slate-200 shrink-0">
            <div class="flex-1">
              <router-link :to="`/post/${selectedOrder.post?.slug}`"
                class="font-bold text-on-surface hover:text-primary transition-colors line-clamp-2 mb-2">{{
                  selectedOrder.post?.title }}</router-link>
              <div class="text-error font-extrabold text-lg">{{ new
                Intl.NumberFormat('vi-VN').format(selectedOrder.total_price || selectedOrder.post?.price) }}đ</div>
            </div>
          </div>
        </div>
      ...
      <div class="p-6 border-t border-slate-100 flex justify-end gap-3 bg-slate-50/50">
        <button @click="closeOrderDetails"
          class="px-6 py-2.5 border border-slate-300 text-on-surface-variant font-bold rounded-xl hover:bg-slate-200 transition-colors cursor-pointer">
          Đóng
        </button>
      </div>
    </div>
  ...
  <!-- Review Modal -->
  <ReviewModal :isOpen="isReviewModalOpen" :orderId="selectedOrderForReview?.id"
    :sellerId="selectedOrderForReview?.seller?.id" :existingReview="existingReviewForOrder"
    @close="isReviewModalOpen = false" @success="handleReviewSuccess" />
</template>
...
const existingReviewForOrder = ref(null);
...
const openReviewModal = (order) => {
  ...
  existingReviewForOrder.value = order.review || null;
  isReviewModalOpen.value = true;
};

const handleReviewSuccess = (review) => {
  if (selectedOrderForReview.value) {
    selectedOrderForReview.value.review = review;
  }
