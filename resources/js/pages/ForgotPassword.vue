<template>
    <div class="bg-surface-container-low rounded-xl p-8 sm:p-12 relative shadow-[0_32px_64px_rgba(28,27,31,0.06)]">
        ...
        <form @submit.prevent="handleForgotPassword" class="flex flex-col gap-6">
            ...
            <div v-if="statusMessage" :class="['text-sm font-bold p-4 rounded-lg', isSuccess ? 'bg-[#d1e7dd] text-[#0f5132]' : 'bg-error-container text-error']">
                {{ statusMessage }}
            </div>
        ...
        <p class="mt-10 text-center text-body-md text-on-surface-variant">
            Nhớ ra mật khẩu rồi? <router-link class="font-bold text-primary hover:text-secondary transition-colors underline"
                to="/login">Đăng nhập</router-link>
        </p>
    </div>
</template>
