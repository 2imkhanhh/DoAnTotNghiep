...
@section('content')
    <div class="bg-surface-container-low rounded-xl p-8 sm:p-12 relative shadow-[0_32px_64px_rgba(28,27,31,0.06)]">
        <div class="mb-10">
            <h1 class="text-3xl font-headline text-primary font-extrabold tracking-[-0.02em] leading-tight mb-2">Quên mật
                khẩu</h1>
            <p class="text-body-lg text-on-surface-variant">Đừng lo lắng! Vui lòng nhập email của bạn, chúng tôi sẽ gửi liên
                kết khôi phục.</p>
        </div>
        ...
        <form id="forgot-password-form" class="flex flex-col gap-6">
            <div class="flex flex-col gap-4">
                <label class="flex flex-col">
                    <span class="text-label-md font-bold text-primary mb-2 tracking-wide uppercase">Địa chỉ Email đã đăng
                        ký</span>
        ...
        <p class="mt-10 text-center text-body-md text-on-surface-variant">
            Nhớ ra mật khẩu rồi? <a class="font-bold text-primary hover:text-secondary transition-colors underline"
...
@section('scripts')
    <script>
        document.getElementById('forgot-password-form').addEventListener('submit', async function(e) {
            ...
            btn.innerHTML =
                '<span>Đang gửi...</span><span class="material-symbols-outlined animate-spin">sync</span>';
            ...
            try {
                ...
                statusDiv.classList.remove('hidden', 'bg-error-container', 'text-error', 'bg-[#d1e7dd]',
                    'text-[#0f5132]');
                ...
                if (response.ok) {
                    ...
                    statusDiv.innerText = data.errors ? Object.values(data.errors)[0][0] : (data.message ||
                        'Có lỗi xảy ra.');
                }
            } catch (error) {
                ...
                btn.innerHTML =
                    '<span>Gửi liên kết khôi phục</span><span class="material-symbols-outlined">mark_email_read</span>';
                btn.style.opacity = '1';
                btn.disabled = false;
            }
        });
