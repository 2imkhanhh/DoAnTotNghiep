...
@section('content')
    <div class="bg-surface-container-low rounded-xl p-8 sm:p-12 relative shadow-[0_32px_64px_rgba(28,27,31,0.06)]">
        ...
        <form id="login-form" class="flex flex-col gap-6">
            <div class="flex flex-col gap-4">
                <label class="flex flex-col">
                    ...
                    <input id="login-email" class="w-full h-14 bg-surface-container-highest border-none rounded-lg px-4 py-3 focus:ring-0 focus:outline-none" placeholder="Nhập email của bạn" required="" type="email"/>
                </label>
                <label class="flex flex-col relative">
                    <div class="flex justify-between items-center mb-2">
                        <span class="text-label-md font-bold text-primary tracking-wide uppercase">Mật khẩu</span>
                        <a href="/forgot-password" class="text-sm font-bold text-primary hover:text-secondary transition-colors" tabindex="-1">
                            Quên mật khẩu?
                        </a>
                    </div>
                    <input id="login-password" class="w-full h-14 bg-surface-container-highest border-none rounded-lg px-4 py-3 focus:ring-0 focus:outline-none" placeholder="Nhập mật khẩu" required="" type="password"/>
                </label>
            </div>
        ...
        <p class="mt-10 text-center text-body-md text-on-surface-variant">
            Chưa có tài khoản? <a class="font-bold text-primary hover:text-secondary transition-colors underline" href="/register">Đăng ký ngay</a>
        </p>
    </div>
@endsection
        ...
        document.getElementById('login-form').addEventListener('submit', async function(e) {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    errorDiv.innerText = data.errors ? Object.values(data.errors)[0][0] : (data.error || 'Đăng nhập thất bại.');
                }
            } catch (error) {
                errorDiv.classList.remove('hidden');
