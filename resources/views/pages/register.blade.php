    ...
    <div class="bg-surface-container-low rounded-xl p-8 sm:p-12 relative shadow-[0_32px_64px_rgba(28,27,31,0.06)]">
        <div class="mb-10">
            <h1 class="text-3xl font-headline text-primary font-extrabold tracking-[-0.02em] leading-tight mb-2">Tạo tài
                khoản</h1>
            <p class="text-body-lg text-on-surface-variant">Bắt đầu hành trình sưu tầm những món đồ độc bản của riêng bạn.
            </p>
        </div>
        ...
        <p class="mt-10 text-center text-body-md text-on-surface-variant">
            Đã có tài khoản? <a class="font-bold text-primary hover:text-secondary transition-colors underline"
        ...
        document.getElementById('register-form').addEventListener('submit', async function(e) {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    errorDiv.innerText = data.errors ? Object.values(data.errors)[0][0] : (data.message ||
                        'Có lỗi xảy ra.');
                }
            } catch (error) {
                errorDiv.classList.remove('hidden');
